...
\begin{algorithm}[htb]
    ...
    \begin{algorithmic}[1]
        ...
        \For{$\ell = 1$ to $L$} 
            \State $\mathcal{V}^{\ell} \gets \text{LS}(\mathcal{H}(\cdot), \mathca{U}^{\ell -1}, \mathbf{y}, r)$
            \State $\mathcal{U}^{\ell} \gets \text{LS}(\mathcal{H}(\cdot), \mathca{V}^{\ell}, \mathbf{y}, r)$
            \EndFor
...
\begin{algorithm}[htb]
    ...
    \State \begin{eqnarray*} \widehat{\mathcal{V}} = \mathop{\arg\min}_{ \mathcal{V} \in \mathbb{R}^{r \times N_2 \times N_3}}\| \mathbf{y} - \mathcal{H}(\mathcal{U} \bullet \mathcal{V}) \|_F^2 , 
    \end{eqnarray*}
...
\begin{equation}
    \mathbf{X}^c = \mathbf{U}^c\mathabf{V}^c,
\end{equation}
