...
\begin{algorithm}[htb]
    ...
    \begin{algorithmic}[1]
        \Require
        linear map $\mathcal{H}(\bullet)$, measurement vector $\mathbf{y}$, $\mathbf{L}$-rank $r$, iteration number $L$.
        \State Initialize $\mathcal{U}^0$ randomly;
        \For{$\ell = 1$ to $L$} 
            \State $\mathcal{V}^{\ell} \gets \text{LS}(\mathcal{H}(\bullet), \mathca{U}^{\ell -1}, \mathbf{y}, r)$
            \State $\mathcal{U}^{\ell} \gets \text{LS}(\mathcal{H}(\bullet), \mathca{V}^{\ell}, \mathbf{y}, r)$
            \EndFor
